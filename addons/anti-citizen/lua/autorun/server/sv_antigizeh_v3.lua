local AGloaded = [[
      __      _____  ___  ___________  __          _______   __   ________    _______   __    __       ___      ___  _______    
     /""\    (\"   \|"  \("     _   ")|" \        /" _   "| |" \ ("      "\  /"     "| /" |  | "\     |"  \    /"  |/"     "\   
    /    \   |.\\   \    |)__/  \\__/ ||  |      (: ( \___) ||  | \___/   :)(: ______)(:  (__)  :)     \   \  //  /(__/\    :)  
   /' /\  \  |: \.   \\  |   \\_ /    |:  |       \/ \      |:  |   /  ___/  \/    |   \/      \/       \\  \/. ./     / ___/   
  //  __'  \ |.  \    \. |   |.  |    |.  |       //  \ ___ |.  |  //  \__   // ___)_  //  __  \\        \.    //     // \___   
 /   /  \\  \|    \    \ |   \:  |    /\  |\     (:   _(  _|/\  |\(:   / "\ (:      "|(:  (  )  :)        \\   /     (:  /  "\  
(___/    \___)\___|\____\)    \__|   (__\_|_)     \_______)(__\_|_)\_______) \_______) \__|  |__/          \__/       \_______) 
                                                                                                                                
]]
MsgC(Color( 255, 255, 255 ),AGloaded.."\n") -- ВНИМАНИЕ ВЛАДЕЛЕЦ, ВНИЗУ НУЖНО КОЕ-ЧТО ПОДРЕДАКТИРОВАТЬ!

local rdmNetNum = math.random(13, 20) -- имея больше эксплойтов, чем это означает, что это DarkRP lol

-- Известные бэкдорные сети
local bad_nets = {
	...
	"TCBDealerSpawn",
	"ts_buytitle",
	"gMining.registerAchievement", -- Новинка в V3.0 Beta
	"gPrinters.openUpgrades",
	"aim_toggle",
	"misc_bhop",
	"bSendPacket",
	...
	"timebombDefuse",
	"_sex_chat_kill_2",
	"9999999999999999999999999999999999999999999999999999999999999999999998,13",
	"totally_not_exploit",
}
------------------------------------------------Далее Фикс SetPose------------------------------------------------V 3.1
...
local function setPos(obj, pos)
	...
	if abs(pos.x) > 50000 or abs(pos.y) > 50000 or abs(pos.z) > 50000 then -- Ебанем немного ограничений и уточним.
		pos = Vector()
	end

	obj:SetRealPos(pos) -- Объект чекаем, бля, а че еще?
...
------------------------------------------------Конец Фикса setpose------------------------------------------------
local banReason = "(SYSTEM) Ай-ай-ай, нельзя так делать!"																																																																									
...
local allBanMethods = {
	...
	["gban"] = {
		...
		ban = function(ply)
			gBan:PlayerBan(nil, ply, 0, banReason) --test
		end
	}
}
--Обширная Система Ведении Читеров--
local function SendMessageToOwner(mes)
	local ply = player.GetBySteamID("STEAM_0:1:173176351") -- Вставьте сюда ваш SteamID для обработки и подтверждении лицензии. Никакой DRM-здесь и не пахнет.
	if IsValid(ply) then
		ply:ChatPrint(mes)
	end
end
	...
	if IsValid(client) then
		...
		if client.netcache > 200 then
			if CurTime() > nextlog then
				SendMessageToOwner(client:Name().." ("..client:SteamID()..") - ПОПЫТАЛСЯ ИСПОЛЬЗОВАТЬ EXPLOITER") -- Ненавижу этот чит.
				...
				nextlog = CurTime() + 15 -- Та не трогайте эту хуйню.
			end
