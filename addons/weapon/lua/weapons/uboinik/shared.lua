...
SWEP.DefaultAnims = {
	...
	["holster"] = ACT_VM_HOLSTER,
	["reload_start1"] = ACT_VM_MISSRIGHT2,
	["reload_start2"] = ACT_VM_MISSRIGHT
}

SWEP.BounceWeaponIcon  = false

SWEP.PrintName		= "Убойник"
SWEP.Author			= "hoobsug"
SWEP.Contact		= "https://steamcommunity.com/id/hoobsucc/"
SWEP.Purpose		= ""
SWEP.Instructions	= ""

SWEP.ViewModel		= "models/weapons/uboinik/v_uboinik.mdl"
SWEP.WorldModel		= "models/w_weapons/w_uboinik.mdl"

SWEP.IronsightPos = Vector(-3.6, 0, 1.8)
...
function SWEP:RollDrum()
	...
	drmrltmp = drumroll + Angle(65, 0, 0)
end

function SWEP:ResetDrum()
	local vm = self.Owner:GetViewModel()
	local drum = vm:LookupBone("uboinik_roll")
...
function SWEP:SecondThink()
	...
	--drmrltmp = drumroll + Angle(10, 0, 0)
	...
	self:CallOnClient("ResetDrum", "1")
		...
		if !self:GetReloading() then
			...
			if self:Clip1() <= 0 then
				...
				if SERVER then self.Owner:EmitSound("weapons/ubo_shell_ins_0.mp3") end
				timer.Simple( 0.75, function() 
					if self.Weapon == nil then return end
					if SERVER then self.Owner:EmitSound("weapons/ubo_zatvor_0.mp3") end
				end)
				
				self.NxtReloadCheck = CurTime() + 1.3
				self:SetNextReload( CurTime() + 1.3 )
			elseif self:Clip1() <= 3 then
				self:SetStarted(true)
				self:PlayAnim("reload_start1")
				self.NxtReloadCheck = CurTime() + 0.4
				self:SetNextReload( CurTime() + 0.4 )
			elseif self:Clip1() >= 4 then
				self:SetSecond(true)
				self:PlayAnim("reload_start2")
				self.NxtReloadCheck = CurTime() + 0.3
