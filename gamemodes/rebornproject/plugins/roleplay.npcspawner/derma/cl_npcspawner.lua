...
function PANEL:Setup(entity)
    ...
    self.weaponClassValue = self.entity:GetWeaponClass()
    self.weaponClassText:SetValue(self.entity:GetWeaponClass())
...
function PANEL:Init()
    self:SetSize(ScrW() * 0.45, ScrH() * 0.43)
    ...
    self.weaponClassRow = self.npcSpawnerPanel:AddRow()
    self.weaponClassLabel = self.weaponClassRow:AddLabel(L("npcSpawnerWeaponClass"))
    self.weaponClassText = self.weaponClassRow:Add("ixTextEntry")
    self.weaponClassText:Dock(RIGHT)
    self.weaponClassText:SetFont("ixMenuButtonFont")
    self.weaponClassText:SetBackgroundColor(derma.GetColor("DarkerBackground", self))
    self.weaponClassText:SetWide(self:GetWide() * 0.4)
    self.weaponClassText.OnChange = function(panel)
        self.weaponClassValue = panel:GetValue()
        self.weaponClassRow:SetShowReset(panel:GetValue() != self.entity:GetWeaponClass(), L("npcSpawnerWeaponClass"), self.entity:GetWeaponClass())
    end
    self.weaponClassRow.OnResetClicked = function()
        self.weaponClassRow:SetShowReset(false)
        self.weaponClassText:SetValue(self.entity:GetWeaponClass())
    end
...
function PANEL:SaveChanges()
    ...
    net.Start("npcSpawnerSaveChanges")
        ...
        net.WriteString(self.weaponClassValue)
