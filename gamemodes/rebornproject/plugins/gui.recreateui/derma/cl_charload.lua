...
function PANEL:ResetSequence(model, lastModel)
    ...
    local sec_id = model:LookupSequence(m[math.random(#m)])  
	model:ResetSequence(sec_id)
	return sec_id
...
function PANEL:Init()
	...
	continueButton.DoClick = function()
		if LocalPlayer():GetNetVar("CanPickChar") == false then return parent:ShowNotice(3, L("Подождите немного...")) end
	...
	deleteConfirm.DoClick = function()
		...
		local moneys = ix.config.Get("Money to remove", 100) -- yes
		local banned = self.character:GetData("banned") -- yes!!!!

		if self.character:GetMoney() >= moneys or banned then -- and yes
			...
			self:SetActiveSubpanel("main")
            parent:ShowNotice(3, L("Невозможно удалить персонажа, так как на нем недостаточно средств!"))
        end
	end
