	...
	ix.char.RegisterVar("model", {
		...
		OnDisplay = function(self, container, payload)
			...
			scroll.Paint = function(panel, width, height)
				...
				draw.RoundedBox(0, 0, 0, width, height, Color(255, 255, 255))
			end
...
do
	if (SERVER) then
		...
		util.AddNetworkString("SH_Pendulum_Propane_Nightmares") -- нетстримы нормально не создаются здесь блять.
		...
		net.Receive("ixCharacterCreate", function(length, client)
			...
			--------------------
			local PlayerScale = net.ReadString()
			--------------------
			...
			ix.char.Create(payload, function(id)
				if (IsValid(client)) then
					...
					local charid = ix.char.loaded[id]
					...
					hook.Run("AnyAct_LockedClub_RLGN", client, ix.char.loaded[id], PlayerScale, loadout) -- You're All Going To Die Down Here
					
					--netstream.Hook("SH_Pendulum_Propane_Nightmares", function(_, client, data, datat) 
						--hook.Run("Absolute_Territory", client, ix.char.loaded[id], data, datat)
					--end)

					-- net.Receive("SH_Pendulum_Propane_Nightmares", function(len, client) -- из-за ивентаря пришлось ебашить нетвары, увы.
					-- 	local data = net.ReadString()
					-- 	local datat = net.ReadString()

					-- 	client:SetNetVar("silly", client)
					-- 	client:SetNetVar("cats", data)
					-- 	client:SetNetVar("for_silly_ppl", datat)
					-- 	--hook.Run("Absolute_Territory", client, charid, data, datat)
					-- end)

					net.Receive("SH_Pendulum_Propane_Nightmares", function(len, client)
						local dataReceived = net.ReadTable()
						client:SetNetVar("silly", client)
						client:SetNetVar("cats", dataReceived)
					end)
