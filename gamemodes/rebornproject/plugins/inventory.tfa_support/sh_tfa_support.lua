...
if CLIENT then
	function PLUGIN:PopulateItemTooltip( tooltip, item )
		if item.Attachments and not table.IsEmpty( item.Attachments ) then
			local text = "Допустимые модификации: "
...
function PLUGIN:InitializedPlugins()
	for k, v in next, weapons.GetList() do
		...
		local ITEM = ix.item.Register( class, "base_weapon_tfa", nil, nil, true )
		...
		ITEM.inventoryType = dat.inventoryType or "weapon_primary"
	...
	for k, v in next, self.AttachData do
		...
		ITEM.exRender = v.exRender or false
	...
	for k, v in next, self.AmmoData do
		...
		ITEM:Hook( "drop", function(item)
			item.player:EmitSound( "eftsounds/gear_helmet_drop.wav" ) 
		end )
	end
end
