...
function character:Recognize(id,NameToReg)
    ...
    local CurFakeNames = self:GetData("fkname_KnowFakeNames",{})

    CurFakeNames[id] = NameToReg

    self:SetData("fkname_KnowFakeNames",CurFakeNames)
...
netstream.Hook("fkname_ActionName",function(player,table)
	if !isstring(table[1]) or table[1] == player:Name() then return end
	local char = player:GetCharacter()
	if !char then return end
	local usednames = char:GetData("fkname_UsedNames",{})
	usednames[table[1]] = !table[2] or nil
	char:SetData("fkname_UsedNames",usednames)
end)