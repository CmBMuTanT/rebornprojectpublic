...
function PANEL:Init()
	...
	if (!suppress.characterInfo) then
        ...
        if (!suppress.national) then
			self.national = self.characterInfo:Add("ixListRow")
			self.national:SetList(self.characterInfo.list)
...
function PANEL:Update(character)
	...
	if ( character:GetData("CharNational") ) then
		self.national:SetLabelText("Национальность")
		self.national:SetText(character:GetData("CharNational"))
		self.national:SizeToContents()
	end
