...
function PANEL:Init()
	...
	self.confirmPanel.Paint = function(this, width, height)
		if (this.confirmTrade) then
			...
			ix.util.DrawText("Готов к торговле.", width / 2, 4, color_white, TEXT_ALIGN_CENTER)
		else
			surface.SetDrawColor(self.cGray)
			surface.DrawRect(0, 4, width, height)

			ix.util.DrawText("Не готов торговать.", width / 2, 4, color_white, TEXT_ALIGN_CENTER)
		end
	end
	...
	self.confirmButton.Reset = function(this)
		...
		this:SetTooltip("Удерживайте эту кнопку, когда будете готовы торговать.")
		this:SetCursor("hand")
	end
	...
	self.confirmButton.Paint = function(this, w, h)
		if (!this:GetDisabled()) then 
			...
			ix.util.DrawText("Принять", w / 2, 5, color_white, TEXT_ALIGN_CENTER)
		else
			surface.SetDrawColor(self.cGreen)
			surface.DrawRect(2, 2, w - 4, h - 4)

			ix.util.DrawText("Принято", w / 2, 5, color_white, TEXT_ALIGN_CENTER)
		end
	end
