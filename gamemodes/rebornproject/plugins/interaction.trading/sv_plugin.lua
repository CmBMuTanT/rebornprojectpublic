...
function ix.trade.Complete(client, clientCharacter, otherCharacter, bMoneyOther, invID)
	client.tradingItems = client.tradingItems or {}
	if (!table.IsEmpty(client.tradingItems) and ix.inventory.Get(invID)) then
		local items = client:GetItems()

		if (items) then
			for id, v in pairs(items) do
				if (client.tradingItems[id]) then
	...
	ix.trade.ClearVars(client)
	client:EmitSound("buttons/button5.wav", 75, 100, 0.5)
end
	...
	if (client.tradeConfirmed and other.tradeConfirmed) then
		...
		timer.Simple(0, function()
			ProtectedCall(function()
				ix.trade.Complete(client, clientCharacter, otherCharacter, false, otherCharacter:GetInventory():GetID())
			end)
		end)

		timer.Simple(0, function()
			ProtectedCall(function()
				ix.trade.Complete(other, clientCharacter, otherCharacter, true, clientCharacter:GetInventory():GetID())
			end)
		end)
