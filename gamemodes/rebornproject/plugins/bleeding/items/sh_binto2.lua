...
ITEM.description = 'Кустарный бинт для остановки кровотечения и предотвращения попадания в рану инфекции. При обычном кровотечении позволяет перевязать рану, при артериальном - перетянуть артерию.'
...
--	sound = "zwa/heal.mp3",
	OnRun = function(item)
		...
		player:SetAction("Использую...", 3.5, function()
			...
			player:SetHealth(math.min(player:Health() + 6, 100))
		end)
		return false
	end,
