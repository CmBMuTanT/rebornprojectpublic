ITEM.name = "Боевой комплект «ТЕЧЬ»"
ITEM.description = "Данный боевой комплект был полностью спроектирован на основе спартанского бронекомплекта «МИРОТВОРЕЦ», за что у Красной Линии создавались внешнеполитические разногласия с Арбатской и Орденом - ведь это была фактическая кража чужих разработок, благо руководство двух государств нашли общий язык. Добавить касаемо самого бронекомплекта нечего - все осталось неизменным, обычный полевой бронекомплект около-среднего класса, разве перекрашенный и с новыми нашивками.. Солдатами оценивается как не особо удобный, давящий и стягивающий - посему особой радости среди кругов ВС РККА данный бронекомплект не вызывает.."
ITEM.category = "[EQ] BODY"
ITEM.exRender = true
ITEM.model = "models/hardbass/stalker_bandit_1_lrazgryz.mdl"
ITEM.width = 2
ITEM.height = 3
ITEM.iconCam = {
	pos = Vector(714.24, -8.23, 169.25),
	ang = Angle(13.34, 179.32, 0),
	fov = 1.6
}
ITEM.Strengthneed = 8
ITEM.width = 2
ITEM.height = 3
ITEM.weight = 4.5
ITEM.damage = {
    0.6, -- Пулестойкость
    0.6, -- Защита от порезов
    0.9, -- Электрозащита
    0.9, -- Термозащита
    0.9, -- Радиозащита
    0.9, -- Химзащита
    0.7, -- Взрывчатка
    0.7, -- Защита от падения
}
ITEM.HitGroupScaleDmg = {
    [HITGROUP_CHEST] = true,
    [HITGROUP_STOMACH] = true,
}

ITEM.LookupBone = "bip01_spine"
ITEM.CamPosVec = Vector(-32, 0, 0)
ITEM.Warm = 65
ITEM.quality = "uncommon"
ITEM.inventoryType = "armor"
ITEM.bodyGroups = {
    ["Тело"] = 15,
}
ITEM.EQsound = "eftsounds/gear_armor_use.wav"
ITEM.DEQsound = "eftsounds/backpack_drop_generic2 #99388.wav"

--------------------Visuals--------------------
ITEM.outfitCategory = "Армор" -- название категории
ITEM.outfit_name = "Пидорасня" -- название оутфита

----Skin Gropus----
ITEM.SGname = "sg_sparta_1_telo" -- название SkinGroup
ITEM.skingroupar = 1 -- значение SkinGroup

function ITEM:RSO_Equip(client)
    client:SetNWInt(self.SGname, self.skingroupar)
end

function ITEM:RSO_UnEquip(client)
    client:SetNWInt(self.SGname, 0)
end
 
 
 
