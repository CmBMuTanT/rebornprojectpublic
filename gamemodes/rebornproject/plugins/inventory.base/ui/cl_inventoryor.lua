...
function PANEL:OnMousePressed(code)
	if (code == MOUSE_LEFT and self:IsDraggable()) then
		...
		surface.PlaySound( "shaky_flashlight/lean_1.wav" )
		self.clickX, self.clickY = input.GetCursorPos()
	elseif (code == MOUSE_RIGHT and self.DoRightClick) then
		surface.PlaySound( "weapons/universal/uni_weapon_holster_04.wav" )
		self:DoRightClick()
	end
end
		...
		for x, items in pairs(inventory.slots) do
			for y, data in pairs(items) do
				...
				if (item and !IsValid(self.panels[item.id])) then
					local icon = self:AddIcon(item:GetModel() or "models/props_junk/popcan01a.mdl", x, y, item.width, item.height, item:GetSkin())
...
vgui.Register("ixInventoryOR", PANEL, "DFrame")

hook.Add("CreateMenuButtons", "ixInventoryOR", function(tabs)
	...
	tabs["inv"] = {
		...
		Create = function(info, container)
			...
			local panel = canvas:Add("ixInventoryOR")
			panel:SetPos(0, 0)
